console.log("setup wizard of supplier-----------------");
frappe.provide("nona_supplier.setup");
...
frappe.setup.on("before_load", function () {
  nona_supplier.setup.slides_settings.map(frappe.setup.add_slide);
  console.log("frappe.setup.slides----supplier file--------");
  console.log(frappe.setup.slides);
});

nona_supplier.setup.slides_settings = [
  {
    // Brand
    name: "brand 2",
    icon: "fa fa-bookmark",
    title: __("The Brand 2"),
    // help: __('Upload your letter head and logo. (you can edit them later).'),
    fields: [
      {
        fieldtype: "Attach Image",
        fieldname: "attach_logo",
  ...
  },
  {
    ...
    validate: function () {
      ...
      if (me.values.bank_account) {
        frappe.call({
          async: false,
          method:
            "erpnext.accounts.doctype.account.chart_of_accounts.chart_of_accounts.validate_bank_account",
          args: {
    ...
    set_fy_dates: function (slide) {
      ...
      if (country) {
        var fy = nona_supplier.setup.fiscal_years[country];
